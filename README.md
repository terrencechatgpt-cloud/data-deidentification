...
- 掃描影像 PDF 會先在瀏覽器內以 `tesseract.js` OCR，輸出保留原始頁面影像，只覆蓋被去識別化的區域，並加入不可見 OCR 文字層以便搜尋；若公司內網封鎖語言模型下載來源，需先允許該模型資產或改用已含文字層的 PDF。
- 瀏覽器只會儲存「偵測規則設定」（localStorage）；文件內容與編碼表不落地。
- 編碼是 `crypto.getRandomValues` 產生的 6 位十六進位隨機碼，與原文無數學關聯，無法由編碼反推原始值。
- **輸出檔案內不殘留已處理項目的原始敏感文字**：
  - 一般文字 PDF 採文字重建；掃描 PDF 保留原始影像，但會覆蓋每個已處理 OCR 區域，並以替換文字層取代該區域的 OCR 文字；
...
| PDF | 文字層或掃描影像 | `.pdf`；文字 PDF 依原座標重建，掃描 PDF 保留原始頁面影像並以遮罩覆蓋去識別區域 | 掃描影像以繁中／英文 OCR 並加入不可見文字層；OCR 漏抓或誤認仍需人工校對 |
