...
| 合約、採購、供應商文件 | 簽署人、地址、統編、銀行帳號、合約／採購編號、CAP／限量額度、還款／折扣比例 |
| 財務報表、付款資料 | 銀行／匯款帳號、信用卡號、財務金額（含還款／付款）、財務比例、發票／單據號碼、公司與人員識別資訊 |
| 臨床試驗、醫療與安全性資料 | 姓名、受試者／個案編號、病歷／就診號、試驗編號、研究中心、日期 |
| 研發、生產與品質文件 | 產品／化合物代碼、藥品批號／批次、供應商、合約與內部編號 |

財務金額、CAP／限量額度、還款／折扣比例、日期、批號、產品代碼與試驗編號可能是分析所需欄位，也可能是機密或準識別資訊；工具會標示它們，但不替使用者決定是否應移除，必須依資料共享目的與公司 SOP 覆核。
...
- **財務資料規則**：可在欄位語境下偵測銀行／匯款帳號、信用卡號、還款／付款／收入／支出／成本／預算／單價／總額／稅額、CAP／限量額度，以及還款／折扣／利率等財務比例；無相關語境的一般數字與統計百分比不會直接命中，降低誤判。
- **二十二種內建類別**：一般個資、財務識別資訊與藥廠文件識別資訊；規則可停用，自訂規則以 RegExp 新增並即時測試。
...
- 掃描影像 PDF 會先在瀏覽器內以 `tesseract.js` OCR；輸出保留原頁視覺版面，但會把遮罩直接燒入重新產生的頁面影像，不會把未遮罩的原始底圖嵌入輸出，並加入不可見 OCR 文字層以便搜尋。若公司內網封鎖語言模型下載來源，需先允許該模型資產或改用已含文字層的 PDF。
- 瀏覽器只會儲存「偵測規則設定」（localStorage）；文件內容與編碼表不落地。
- 編碼是 `crypto.getRandomValues` 產生的 6 位十六進位隨機碼，與原文無數學關聯，無法由編碼反推原始值。
- **輸出檔案內不殘留已處理項目的原始敏感文字**：
  - 一般文字 PDF 採文字重建；掃描 PDF 會把遮罩燒入重新產生的頁面影像，未遮罩的原始底圖不會保留在輸出檔，並以替換文字層取代該區域的 OCR 文字；
...
| PDF | 文字層或掃描影像 | `.pdf`；文字 PDF 依原座標重建，掃描 PDF 保留原頁視覺版面並把遮罩燒入新頁面影像 | 掃描影像以繁中／英文 OCR 並加入不可見文字層；OCR 漏抓或誤認仍需人工校對 |
...
| 財務金額 | 有財務欄位語境的數字，可含千分位、小數及「億／萬」中文單位 | 需看到還款、付款、CAP、限量額度、收入、支出、成本、預算、單價、總額、稅額、應收／應付等語境；一般數字不會直接命中 |
| 財務比例 | 還款、返還、折扣或利率語境下的百分比 | 支援 OCR 將小數點前後拆開的情況；掃描合約若表格文字被一般 OCR 略過，會在相關頁面追加一次數字／百分比辨識 |
